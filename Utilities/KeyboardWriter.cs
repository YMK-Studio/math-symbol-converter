...
namespace MathSymbolConverter.Utilities
{
    internal class KeyboardWriter
    {
        ...
        public static async void PrintText(string str, int aliasLength)
        {
            IntPtr hWnd = GetForegroundWindow();

            uint fromId = GetCurrentThreadId();
            uint toId = GetWindowThreadProcessId(hWnd, out _);
            ...
            if (aliasLength != 0)
            {
                for (int i = 0; i < aliasLength + 1; i++)
                {
                    SendMessageW(handle, WM_KEYDOWN, (IntPtr)KeyCode.BACKSPACE, (IntPtr)0);
                    SendMessageW(handle, WM_KEYUP, (IntPtr)KeyCode.BACKSPACE, (IntPtr)1);
                }
            }
            ...
            {
                SendMessageW(handle, WM_CHAR, i, IntPtr.Zero);          
            }

            AttachThreadInput(fromId, toId, false);
        }

        public static void TriggerDefaultTabKeyDownEvent()
        {
            IntPtr hWnd = GetForegroundWindow();
            ...
            PostMessage(handle, WM_KEYDOWN, (IntPtr)KeyCode.TAB, (IntPtr)0);
            //SendMessage(handle, WM_KEYDOWN, (IntPtr)KeyCode.TAB, (IntPtr)0);

            AttachThreadInput(fromId, toId, false);

            //keybd_event((uint)KeyCode.TAB, 0, 0x00, 0);
            
        }

        public static void TriggerDefaultTabKeyUpEvent()
        {
            IntPtr hWnd = GetForegroundWindow();
            ...
            SendMessage(handle, WM_KEYUP, (IntPtr)KeyCode.TAB, (IntPtr)0);

            AttachThreadInput(fromId, toId, false);

            //keybd_event((uint)KeyCode.RIGHT, 0, 0x00, 0);
            //keybd_event((uint)KeyCode.RIGHT, 0, 0x02, 0);

            //keybd_event((uint)KeyCode.TAB, 0, 0x02, 0);
        }
    }
}
